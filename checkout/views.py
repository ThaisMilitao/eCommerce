...
class CreateCartItemView(RedirectView):
    def get_redirect_url(self,*args, **kwargs):
        ...
        if created:
            messages.success(self.request, 'Product adicionado com sucesos')
        else:
            messages.success(self.request, 'Product atualizado com sucesos')
...
class CartItemView(TemplateView):
    ...
    #     return modelformset_factory(CartItem, fields=('quantity',), can_delete=True, extra=0)
    ...
    def post(self, request,*args, **kwargs):
        ...
        if formset.is_valid():
            formset.save()
            messages.success(request,'Carrinho atualixado')
...
class CheckoutView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if session_key and CartItem.objects.filter(cart_key=session_key).exists():
            ...
            messages.info(request,'Não há')
            return redirect('checkout:cart_item')
        return super(CheckoutView, self).get(request, *args, **kwargs)
    
createCartItem = CreateCartItemView.as_view()
