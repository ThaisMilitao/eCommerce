...
class RegisterViewTestCase(TestCase):
    ...
    def test_register_ok(self):
        ...
        index_url = reverse('commerce:home')
...
class UpdateUserTestCase(TestCase):
    ...
    def test_update_user_ok(self):
        ...
        self.assertEquals(self.user.name,'test3')

    def test_update_user_error(self):
        data = {}
        self.client.login(username=self.user.username, password='123')
        response = self.client.post(self.url, data)
        self.assertFormError(response,'form','email', 'Esse camp...')
...
class UpdatePasswordTestCase(TestCase):
    ...
    def test_update_password_ok(self):
        data = {'old_password':'123', 'new_password':'test123', 'new_password2':'test123'}
        self.client.login(username=self.user.username, password='123')
        response = self.client.post(self.url, data)
        self.user.refresh_from_db()
